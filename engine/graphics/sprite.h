			...
			public:
				...
				class Frame
					...
					protected:
						...
						std::map<std::string, Engine::Math::AVector> miscPoints;
					...
					public:
						...
						Engine::Math::AVector getPoint(const std::string& name) const
							{
							auto it=miscPoints.find(name);
							if(it==miscPoints.end())
								return Engine::Math::AVector();

							return it->second;
							}
						inline Engine::Math::AVector getOffset() const {return Engine::Math::AVector(ox, oy);}

						void addPoint(const std::string& name, const Engine::Math::AVector& pos) {miscPoints[name]=pos;}
