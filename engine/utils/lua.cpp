...
bool Lua::init()
	...
	if(!state)
		{
		LOG_ERROR("Lua.init: Nie udalo sie stworzyc state");
...
bool Lua::open(const std::string& path)
	...
	if(!data)
		{
		LOG_ERROR("Lua.open: Nie udalo sie wczytac pliku \"%s\"", path.c_str());
	...
	if(!parse(data))
		{
		LOG_ERROR("Lua.open: Problem przy parsowaniu skryptu \"%s\"", path.c_str());
...
bool Lua::error()
	...
	switch(status)
		{
		case LUA_ERRRUN:
			LOG_ERROR("Lua.error: %s: Problem z wykonaniem: %s", path.c_str(), lua_tostring(state, -1));
		break;

		case LUA_ERRSYNTAX:
			LOG_ERROR("Lua.error: %s: Blad skladni: %s", path.c_str(), lua_tostring(state, -1));
		break;

		case LUA_ERRMEM:
			LOG_ERROR("Lua.error: %s: Problem z alokacja pamieci: %s", path.c_str(), lua_tostring(state, -1));
		break;

		case LUA_ERRGCMM:
			LOG_ERROR("Lua.error: %s: Problem z garbage collectorem: %s", path.c_str(), lua_tostring(state, -1));
		break;

		case LUA_ERRERR:
			LOG_ERROR("Lua.error: %s: Problem obsluga bledu: %s", path.c_str(), lua_tostring(state, -1));
		break;

		case LUA_ERRFILE:
			LOG_ERROR("Lua.error: %s: Problem z wczytaniem pliku: %s", path.c_str(), lua_tostring(state, -1));
		break;
...
bool Lua::addTableStack(const std::string& name)
	...
	LOG_DEBUG("Lua.addTableStack: [%s : %d]", name.c_str(), pars.count());
