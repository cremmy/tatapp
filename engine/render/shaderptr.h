		...
		class ShaderPtr
			...
			private:
				struct Uniform
					...
					Uniform(const Engine::Math::AVector& v): type(Type::VECTOR)
						{
						value.uvec[0]=v.x;
						value.uvec[1]=v.y;
						value.uvec[2]=v.z;
					...
					Uniform(const Engine::Math::AQuaternion& v): type(Type::QUATERNION)
						{
						value.uquat[0]=v.r;
						value.uquat[1]=v.i;
						value.uquat[2]=v.j;
					...
					Uniform(const Engine::Math::AMatrix& v): type(Type::MATRIX)
						{
						value.umatr[ 0]=v[0][0];
						value.umatr[ 1]=v[0][1];
						value.umatr[ 2]=v[0][2];
			...
			public:
				...
				void uniform(const std::string& name, const Engine::Math::AVector& v) {uvalues[shader->getUniform(name)]=Uniform(v);}
				void uniform(const std::string& name, const Engine::Math::AQuaternion& v) {uvalues[shader->getUniform(name)]=Uniform(v);}
				void uniform(const std::string& name, const Engine::Math::AMatrix& v) {uvalues[shader->getUniform(name)]=Uniform(v);}
